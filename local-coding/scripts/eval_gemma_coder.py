...
out=root/'data'/('gemma-eval-'+time.strftime('%Y%m%d-%H%M%S'))
out.mkdir(parents=True)
model=root/'data/models/gemma4-coder-q3/gemma4-coding-Q3_K_M.gguf'
...
if not model.exists() or not (model.parent/'verified.json').exists(): raise RuntimeError('Unverified model')
report['manifest']=json.loads((model.parent/'verified.json').read_text())
...
args=['Z:/models/runtime/llama-vulkan/llama-server.exe','-m',str(model),'--host','127.0.0.1','--port','9103','--alias','gemma4-coder-q3','--device','Vulkan1','-ngl','999','-c','8192','-np','1','-ctk','q8_0','-ctv','q8_0','-fa','on','--jinja','--no-warmup','--metrics','--no-mmap','-b','256','-ub','128','-lv','4']
report['args']=args
...
try:
    ...
    with httpx.Client(base_url='http://127.0.0.1:9103',timeout=180) as c:
        ...
        def request(label,messages,max_tokens=1024):
            ...
            r=c.post('/v1/chat/completions',json={'model':'gemma4-coder-q3','messages':messages,'temperature':0,'max_tokens':max_tokens,'chat_template_kwargs':{'enable_thinking':False}})
