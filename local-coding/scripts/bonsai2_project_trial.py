"""One bounded, source-only Bonsai 2 project trial; no automatic code execution.
Isolated Prism-runtime copy of bonsai_project_trial.py; original untouched."""
...
parser.add_argument('--model', type=Path, default=ROOT/'data/models/bonsai2-27b-ptq10/Ternary-Bonsai-2-27B-PTQ1_0.gguf')
parser.add_argument('--label', default='bonsai2')
...
def main():
    ...
    for p in psutil.process_iter(['pid','name','exe','cmdline','create_time']):
        ...
        cmd = info['cmdline'] or []
        if '8123' in cmd and any('qwen2.5-coder-7b-instruct-q4_k_m.gguf' in a for a in cmd):
            if prior: raise RuntimeError('Unexpected active model; refusing overlap')
            prior = info
            continue
        report.setdefault('unrelated_servers', []).append({'pid': info['pid'], 'exe': info['exe'], 'cmdline': cmd})
    ...
    try:
        ...
        report['context'] = 8192 if not opts.probe_tools else 4096
        args = ['Z:/Models/runtime/llama-prism-b10685-vulkan/llama-server.exe','-m',str(MODEL),'--host','127.0.0.1','--port','9103','--alias','bonsai2-trial','--device','Vulkan1','-ngl','60','-c',str(report['context']),'-np','1','-ctk','q8_0','-ctv','q8_0','-fa','on','--jinja','--no-warmup','-b','256','-ub','128']
        if opts.nanbeige_thinking:
        ...
        with httpx.Client(base_url='http://127.0.0.1:9103',timeout=180) as client:
            ...
            if opts.probe_tools:
                probe = client.post('/v1/chat/completions', json={
                    'model':'bonsai2-trial','messages':[{'role':'user','content':'Call read_file with path AGENTS.md now. Do not answer in prose.'}],
            ...
            for turn in range(24 if opts.test_feedback else 16):
                ...
                response=client.post('/v1/chat/completions',json={'model':'bonsai2-trial','messages':messages,'tools':tools,**sampling,'max_tokens':8192 if opts.nanbeige_thinking else 2200,'chat_template_kwargs':template_kwargs},timeout=httpx.Timeout(remaining)).raise_for_status().json()
