...
|---|---|---|---|---|---|---|---|---|
| Bonsai-2 27B TQ2_0 (bonsai2) | 9103 | 6.48 GiB / ~5.0 GB | **13.0 logger-attached / ~15 est. clean (E2 int-dot vecq 9/25: GPU-sum 81→53 ms/tok −35%); 12.7 @32K q4 KV; 8–9 contended** | 8K (100K = 3.2 t/s, skip; 32K via q4_0 KV = 10.9 t/s @16k fill, pp 70 t/s) | q8 KV, ngl 99, --reasoning-effort medium (xhigh template default = empty answers @4k, see BONSAI2_REASONING_EFFORT.md); json_object truncates | 3 GB | yes | smartest local; golden-check vs CUDA T4 CERRADO 2026-09-22: 4/8 prompts divergen (siempre al token rank-2/3 del golden, texto aun correcto) = fidelity gap, no corrupcion |
| Bonsai-2 27B PTQ1_0 | 9103 | ~5.9 GB | **7.4–7.9 (was 6.4–6.7; E2 int8-LUT 9/24: GPU-sum 157.8→129.2 ms/tok, −18%)** | 8K, 16K+ viable (~1 GB less VRAM than TQ2) | E2 int8 LUT in BOTH paths (f32 + vecq, LDS 5→1.25 KB), golden 7/8 intact 9/24; fix lives in C:/src build, NOT in shipped b10685 runtime | 3 GB | yes | fidelity path (golden 7/8); TQ2_0 ~1.7x faster; upstream-PR material now incl. int8-LUT + vecq repair |
| NeoHorse-1 4B Q8 (neohorse) | 9107 | ~4.5 GB | 36–37 short; 29 @31K, 24 @64K, 21 @130K | launcher starts 32768 since 2026-09-23 (16384 was too small: an omp agent prompt is ~21.4K tokens before the user writes anything); 32K/64K/131K VALIDATED 2026-09-20 (needle PASS at all rungs; 131K needs q4 KV) | prefill ~300 t/s @31K, 166 @64K, 91 @130K → first token 2 min / 6.5 min / 24 min; VRAM dedicated 4.98 GB @32K q8 KV | ~1.5 GB | yes 3/3 | fast tier; long-ctx champ; router candidate |
